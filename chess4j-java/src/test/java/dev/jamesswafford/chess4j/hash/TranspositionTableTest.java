...
public class TranspositionTableTest {
    ...
    public void storePromotion() {
        ...
        ttable.store(board, EXACT_SCORE,100,3,promotion);
        assertEquals(promotion, ttable.probe(board).getMove());
    }
    ...
    public void storeCastle() {
        ...
        ttable.store(board, EXACT_SCORE,100,3,castle);
        assertEquals(castle, ttable.probe(board).getMove());
    }
    ...
    public void storeEPCapture() {
        ...
        ttable.store(board, EXACT_SCORE,100,3,epCapture);
        assertEquals(epCapture, ttable.probe(board).getMove());
    }
    ...
    public void storeMateScore() {
        ...
        ttable.store(board, LOWER_BOUND,CHECKMATE-3, 5, m);
    ...
    public void transformUpperBoundMate() {
        ...
        ttable.store(board, UPPER_BOUND, CHECKMATE-3, 5, m);
        TranspositionTableEntry tte = ttable.probe(board);
    ...
    public void transformExactScoreMate() {
        ...
        ttable.store(board, EXACT_SCORE,CHECKMATE-7, 5, m);
        TranspositionTableEntry tte = ttable.probe(board);
        assertEquals(LOWER_BOUND, tte.getType());
        assertEquals(m, tte.getMove());
    ...
    public void transformLowerBoundMated() {
        ...
        ttable.store(board,LOWER_BOUND,-CHECKMATE+10, 8, m);
        TranspositionTableEntry tte = ttable.probe(board);
        assertEquals(MOVE_ONLY, tte.getType());
    ...
    public void transformExactScoreMated() {
        ...
        ttable.store(board, EXACT_SCORE,-CHECKMATE+12, 10, m);
        TranspositionTableEntry tte = ttable.probe(board);
    ...
    public void transformUpperBoundMated() {
        ...
        ttable.store(board,UPPER_BOUND,-CHECKMATE+8, 7, m);
        TranspositionTableEntry tte = ttable.probe(board);
        assertEquals(UPPER_BOUND, tte.getType());
        assertEquals(m, tte.getMove());
    ...
    public void clearTable() {
        ...
        ttable.store(board, LOWER_BOUND,-93, 4, m);
        ttable.probe(board);
    ...
    public void overwrite() {
        ...
        ttable.store(board, LOWER_BOUND,1001, 5, m);
        TranspositionTableEntry tte = ttable.probe(board);
        TranspositionTableEntry lbe = new TranspositionTableEntry(board.getZobristKey(), LOWER_BOUND,1001,5, m);
        assertEquals(lbe, tte);

        // overwrite with different score/depth
        ttable.store(board, LOWER_BOUND,900, 6, m);
        tte = ttable.probe(board);
        assertNotEquals(lbe, tte);
        lbe = new TranspositionTableEntry(board.getZobristKey(), LOWER_BOUND,900,6, m);
        assertEquals(lbe, tte);

        // use different key to store new entry
        Undo u = board.applyMove(m);
        assertNull(ttable.probe(board));
        ttable.store(board, LOWER_BOUND, 800, 7, m);
        ...
        board.applyMove(m);
        TranspositionTableEntry lbe2 = new TranspositionTableEntry(board.getZobristKey(), LOWER_BOUND,800,7, m);
        tte = ttable.probe(board);
