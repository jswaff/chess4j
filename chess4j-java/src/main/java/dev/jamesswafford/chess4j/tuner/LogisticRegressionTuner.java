...
public class LogisticRegressionTuner {
    ...
    public Tuple2<EvalWeights, Double> optimize(EvalWeights initialWeights, List<FENRecord> dataSet,
        ...
        double initialError = CostFunction.cost(testSet, initialWeights);
        ...
        double finalError = CostFunction.cost(testSet, weights);
    ...
    private EvalWeights trainWithGradientDescent(List<FENRecord> trainingSet, List<FENRecord> testSet,
        ...
        for (int it=0; it<maxIterations; it++) {
            ...
            if ((it+1) % 100 == 0) {
                double trainingError = CostFunction.cost(trainingSet, bestWeights);
                double testError = CostFunction.cost(testSet, bestWeights);
