...
public class BoardUtils {
    ...
    private static boolean isPseudoLegalPawnMove(Board board, Move m) {
        ...
        if (m.captured()==null) {
            ...
            if (board.getPlayerToMove()==Color.WHITE) {
                if (m.isEpCapture()) {
                    return m.to() == board.getEPSquare() && board.getPiece(m.to().south().get()) == Pawn.BLACK_PAWN;
                } else {
                    return board.getPiece(m.to()) == m.captured();
                }
            } else {
                if (m.isEpCapture()) {
                    return m.to() == board.getEPSquare() && board.getPiece(m.to().north().get()) == Pawn.WHITE_PAWN;
                } else {
                    return board.getPiece(m.to()) == m.captured();
                }
            }
